...
export class GrupoComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    if (this.userId) {
      ...
      this.grupoService.getGrupoByUserId(+this.userId).subscribe({
        next: (grupo: Grupo) => {
          ...
          this.grupoForm.patchValue({
            ...
            desplazamiento: grupo.desplazamiento,
  ...
  guardar() {
    ...
    if (this.grupoForm.valid) {
      ...
      if (!/^\d{2}:\d{2}$/.test(grupoData.duracion)) {
        Swal.fire({
          icon: 'error',
          title: 'Error en Duración',
          text: 'La duración debe tener el formato HH:mm.',
        });
        this.spinner = false;
        return;
      }
  ...
  eliminarCliente() {
    if (this.grupoId !== null) {  // Verifica que clienteId no es null
      Swal.fire({
